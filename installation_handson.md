...
```bash
uda_cli -h localhost -p 56565 "IMAS::get(uri='imas:hdf5?dd_version=3.39.0&verbose=1&path=/home/user/data/105029/1', mode='open', dataObject='magnetics', access='read', range='global', time=-999, interp='undefined', path='magnetics/ids_properties/version_put/data_dictionary', datatype='char', rank=1, is_homogeneous=0, dynamic_flags=0)"
```
...
- Leave this task running while performing the following debugging tasks &#8212; if you kill this task then the server will not be running and the following task will fail.
- You will need to open a new terminal and connect to the VM again to run tasks 26-28.
...
- The following task needs to be run in the terminal in which you performed task 25, i.e. where the client was left hanging while attaching the debugger to the server.
